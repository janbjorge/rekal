...
Repeat 3–6 per repo: `tinygrad`, `pytorch`, `fastapi`, `pydantic`.
...
deps (typer) on first run; no venv to manage.
...
it disables auth. Details in DESIGN.md.
...
the agent actually called `memory_store` — the warm arm now has something
...
- Per-role summary: the overhead/benefit/net decomposition, plus a
  **quality** line (`cold=… warm-seed=…`). If warm scored materially worse
  it's flagged `WARM WORSE, savings suspect` — a cheap wrong answer is not
  a win. Run `judge` first or `aggregate` warns that quality is unverified.
