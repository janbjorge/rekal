...
The recall hooks run the plugin's own rekal install (`uv` must be available; no separate `rekal` on the PATH needed). The environment lives in `${CLAUDE_PLUGIN_DATA}/venv` — Claude Code's [persistent plugin data directory](https://code.claude.com/docs/en/plugins-reference#persistent-data-directory), which survives plugin updates. The SessionStart hook syncs it with `uv sync --frozen` only when the bundled `uv.lock` differs from the copy stored beside the venv (first install, or an update that changed dependencies); every hook then runs through `uv run --frozen --no-sync`, so per-turn and per-tool hooks never resolve dependencies or touch the network. Recall never blocks a session: a failed sync (for example, offline first install) skips injection and retries next session, and a missing DB, load error, or empty result degrades to injecting the directive alone. The env-var prefix in the hook commands is POSIX shell syntax; on Windows (where shell-form hooks run under PowerShell) the hooks are currently unsupported.
